...
- PLAY STORE CONFIRMED (her call): Oct 7 both stores. Google new-personal-account rule = closed test, 12 testers enrolled 14 CONTINUOUS days before production access. Plan: Play Console account ($25) NOW alongside Apple enrollment; Android Capacitor build EARLY (~Sept 3-5) to start the 14-day clock (runs Sept 5-19 in parallel with iOS work); apply production + submit both ~Sept 20. HER TASKS: Apple enrollment, Play Console account, recruit 12 Android testers who keep the app installed 14 days.
- [ ] LATER (her call, Aug 25 night, exhausted): group activity sync lag - currently ~15s phone-to-phone with wall open (existing poll cycle), instant on reopen. Fine for launch. If revisited: option 1 = faster/focus-triggered refresh of squad data; option 2 = Supabase Realtime subscriptions (post-launch grade). Do not touch without her go.
- [ ] MONETIZATION LEVER (her call, Aug 28): group member CAP as a paid tier — today groups have NO member limit anywhere (client, join fn, or DB; the 12 in code is only the dense-wall layout switch `is2B`). Plan when revisited: free groups capped at N members (she'll pick N), beyond N requires payment; enforce at join time (client check + a DB-side guard so the API can't bypass) with a clean "this group is full" message. Deliberately NOT built now — launch first.
- [ ] GROUP WALL — EXPLORED AND PARKED (Aug 28 evening, her verdict): the current wall + winning-clip reveal STAYS ("the current design of unveiling winning clip is better"). Full exploration ran: tubes (doer-groupledger-mockup.html), editorial ledgers (doer-groupledger-editorial.html), film rolls (doer-filmroll-mockup.html, doer-filmroll-apple.html), shared strip-wall at 2/6/12 people (doer-sharedwall-mockup.html) — all rejected as less alive than the reveal. PRINCIPLES that survived, binding on future group design: (1) proof is a gift never a duty — no empty photo slots, batch-tickers first-class (her friends' real feedback); (2) group pages need EVENTS not textures — reveals beat tapestries; (3) the one fragment worth revisiting = the live today-scoreboard hero tile (per-member columns filling toward midnight) ADDED above the current wall, not replacing it. Do not reopen without her ask.

## NEXT SESSION (agreed Aug 28 night)
- HER: Apple Developer enrollment ($99) + Play Console account ($25) + start recruiting 12 Android testers. These block everything native; days to clear.
- ME: build REPORT/BLOCK on clips (Apple-mandatory UGC) — first thing, needs nothing from her accounts.
- Today shipped v0605-413→428: apple-quiet active cards (Day hero + wall-window + waiting names), quarter-size trophy wall w/ her 50% COMPLETED/EXPIRED rule, apple-shaped solo page (one + row, suggestion strip, folded COMPLETED row), app-wide type lift (v425), app-wide ink lift + her grey law (v426-428). Revert points: v424 (pre-type-lift), v425 (pre-ink-lift).
- Status telling her: product ~90%, store-readiness ~40%, overall ~75% to two approved stores.
- [ ] ⏰ FIRST THING TOMORROW (her order, Aug 28 night): redo the winning-clip camera flip THE APPLE WAY — kill the "↺ BACK CAMERA" text pill (v429), replace with Apple Camera's control: a small frosted-glass CIRCLE button with the two-arrows-around-a-camera glyph, sitting in the corner of the live preview (Apple puts it bottom-right of the viewfinder), no words, flips on tap with a quick spin. Mockup/preview first, then ship.
- [x] DONE v435 + SQL (Aug 29, assistant ran the dashboard SQL via her Chrome): REPORT/BLOCK complete — hold any non-mine bubble OR winning clip → report sheet; content_flags (review queue: select * from content_flags order by created_at desc) + user_blocks tables live with RLS; client blocklist cache doer_blocked enforced on the wall. Apple UGC requirement satisfied. Remaining nice-to-have: verify the moments Edge Fn's server-side bubble filtering for blocked users on a real two-phone test.

## Invite deep-link — LAUNCH-TIME CHANGES (added Aug 29, v441-442 shipped the web half)
The join link (`?join=CODE` in all three WhatsApp shares) + consent sheet + land-in-room are LIVE on the PWA. When the store apps launch, ADD (do not rebuild):
1. **iOS Universal Links**: host `/.well-known/apple-app-site-association` (JSON, no extension, served as application/json) on the final domain; add Associated Domains entitlement (`applinks:beingdoer.com`) in the Capacitor iOS shell. Then invite links open the installed app instead of Safari.
