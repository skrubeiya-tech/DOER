...
- [ ] BEFORE LAUNCH: self-host Cormorant Garamond + Cinzel woff2 inside the repo (drop the Google Fonts link) so no device ever renders fallback fonts (her Pixel showed plain-serif numbers on first load, Aug 25); add files to SW PRECACHE. Also: one full design review ON ANDROID before launch - rendering is thinner/sharper there; check the places we tuned by thickness (wordmark icon, headers, heading weights).
